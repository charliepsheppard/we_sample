...
router.delete(
  ...
  (req, res) => {
    ...
    // Restaurant.updateOne({ _id: req.query.restaurantId }, { $pull: { samples: { $elemMatch: {_id: req.params.sampleId} } } })
    //   .then(() => res.status(200).json({ msg: "delete from restaurant successful" }))
    //   .catch(err => res.status(404).json({ nosamplefound: err }));
